...
summary: Recuperar una aplicación para un clúster gestionado 
---
= Recuperar una aplicación para un clúster gestionado
:allow-uri-read: 
...
* Longitud mínima: 1
* Longitud máxima: 63
* Esta cadena es validada internamente para prevenir ataques XSS, Unicode, directory traversal y SQL-injection.
...
Matriz JSON de objetos JSON que describen los recursos con alcance de clúster que forman parte de esta aplicación
...
* "borrando" - La aplicación está en proceso de ser eliminada
...
* "protegida" - la aplicación tiene definida una política de protección y es conforme con esa política
* "parcial" - La aplicación tiene definida una política de protección, pero no se ajusta a esa política
...
* "system" - La aplicación contiene un recurso que la identifica como parte del entorno Kubernetes
...
Matriz JSON de objetos JSON, cada uno de los cuales asigna una clase de almacenamiento de origen a una clase de almacenamiento de destino para una operación de restauración. Si no hay elementos o no están presentes, se utilizarán los nombres originales de las clases de almacenamiento de origen. Los valores definidos son:
...
Cadena JSON que contiene el nombre del clúster en el que se ejecutaba anteriormente la aplicación. Solo se devuelve si la aplicación se clonó desde otro clúster.



| sourceClusterID | cadena | Falso  a| 
Cadena JSON que contiene el ID del clúster en el que la aplicación se ejecutaba anteriormente. Solo se devuelve si la aplicación se clonó desde otro clúster. Cuando se especifica en create, el sourceClusterID se usa junto con el sourceAppID e indica el clúster de origen de la aplicación que se va a clonar. Si el clúster de origen especificado no existe, la clonación fallará. Los valores definidos son:
...
Matriz JSON de cadenas JSON que representan un selector de etiquetas para el ámbito de clúster GroupVersionKinds (GVK) dentro del clúster. Si está vacía, se incluyen todas las instancias GVK del clúster. Cada elemento de esta matriz es una cadena de selector de etiquetas de Kubernetes y sigue la misma gramática para esos valores. Ejemplos:
...
* [ "app=mysql,version=2" ] -- Selecciona todos los recursos que tengan una etiqueta cuyo nombre sea app y cuyo valor sea mysql y una etiqueta cuyo nombre sea version y cuyo valor sea 2
